...
describe('SMS send — happy paths', () => {
	it('Form (fields): builds expected payload', async () => {
		...
		expect(body.TextContentTagModels).toEqual([{ Name: '{{firstname}}', Value: 'John' }]);
	});
	...
	it('JSON: parses mergeCodesJson into TagModels', async () => {
		...
		expect(lastCallBody().TextContentTagModels).toEqual([{ Name: '{{firstname}}', Value: 'Jane' }]);
	});
...
describe('SMS send — validation', () => {
	it('throws on invalid destinationNumber (missing "+")', async () => {
		...
		await expect(execute.call(ctx, 0)).rejects.toThrow(/Destination Number/i);
