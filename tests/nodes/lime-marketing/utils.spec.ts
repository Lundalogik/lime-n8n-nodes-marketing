...
describe('assertValidEmail', () => {
    ...
    it.each([
        ...
        expect(() => assertValidEmail(ctx, email, 'Field', 0)).toThrow(
            NodeOperationError
        );
    });
    ...
    it('includes the field label and the bad value in the message', () => {
        expect(() =>
            assertValidEmail(ctx, 'bad', 'Recipient Email', 0)
        ).toThrow(/Recipient Email is not a valid email address: bad/);
    });
});
...
describe('assertValidSmsDestinationNumber', () => {
    ...
    it.each(['+46701234567', '+1234567890', '+999987654321098'])(
        'accepts %s',
        (value) => {
            expect(() =>
                assertValidSmsDestinationNumber(ctx, value, 'Field', 0)
            ).not.toThrow();
        }
    );
...
describe('assertValidSmsFromNumber', () => {
    ...
    it.each([
        ...
        expect(() =>
            assertValidSmsFromNumber(ctx, value, 'Field', 0)
        ).not.toThrow();
    });
    ...
    it.each([
        ...
        expect(() => assertValidSmsFromNumber(ctx, value, 'Field', 0)).toThrow(
            NodeOperationError
        );
    });
});
...
describe('getBaseUrl', () => {
    ...
    it('preserves path segments before stripping the trailing slash', async () => {
        const ctx = makeCredentialsContext(
            'https://app.bwz.se/bedrock/customer/api/'
        );
        expect(await getBaseUrl(ctx)).toBe(
            'https://app.bwz.se/bedrock/customer/api'
        );
    });
});
...
describe('buildAttachment', () => {
    ...
    it('throws when neither override nor upstream filename is set', async () => {
        ...
        await expect(buildAttachment(ctx, 0, 'data')).rejects.toBeInstanceOf(
            NodeOperationError
        );
        await expect(buildAttachment(ctx, 0, 'data')).rejects.toThrow(
            /missing a file name/
        );
    });
});
