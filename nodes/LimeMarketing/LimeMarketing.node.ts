...
import { transactionMailFields, transactionMailOperations } from './resources/transactionmail';
...
function hasJsonField(value: IDataObject | INodeExecutionData): value is INodeExecutionData {
...
function toExecutionItem(raw: IDataObject | INodeExecutionData, i: number): INodeExecutionData {
...
// of execution items.
function toExecutionItems(responseData: OperationExecutorResult, i: number): INodeExecutionData[] {
...
function buildErrorItem(error: Error, i: number): INodeExecutionData {
	const nodeError =
		error instanceof NodeApiError || error instanceof NodeOperationError ? error : undefined;
...
export class LimeMarketing implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Send transactional emails and SMS via the Lime CRM Marketing API',
		defaults: { name: 'Lime CRM Marketing' },
		inputs: [NodeConnectionTypes.Main],
		outputs: [NodeConnectionTypes.Main],
		credentials: [{ name: LIME_MARKETING_API_CREDENTIAL_KEY, required: true }],
		usableAsTool: true,
		properties: [
			{
				displayName:
